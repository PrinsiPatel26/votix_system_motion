...
export function BottomEntrySeriesDetail() {
  const { slug } = useParams<{ slug: string }>();
  const series = slug ? getBottomEntrySeries(slug) : undefined;
  const [activeTab, setActiveTab] = useState<string>('Overview');

  usePageMeta(
    series ? `VOTIX Systems | ${series.name}` : 'VOTIX Systems | Product not found',
    series?.description ?? 'The requested bottom entry series could not be found.'
  );
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-steel-200 bg-white p-5">
          ...
          <ul className="mt-4 space-y-2 text-sm leading-relaxed text-steel-600">
            <li>• Bottom-entry mounting arrangement matched to vessel geometry</li>
  ...
  return (
    <>
      ...
      <CategorySeriesSection category="bottom-entry" />
