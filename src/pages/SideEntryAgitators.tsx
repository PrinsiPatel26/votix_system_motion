  ...
  return (
    <>
      <PageHero
        ...
        title="ENGINEERED SIDE ENTRY MIXING."
        description="Compact. Efficient. Easy to Maintain. VOTIX Side Entry Agitators are designed for reliable performance in a wide range of mixing applications where top entry is not practical."
        image="/ChatGPT%20Image%20Aug%2031,%202026,%2010_00_09%20AM-v.png"
        imageAlt="Side entry agitator system"
        crumbs={[{ label: 'Products', href: '/products' }, { label: 'Side Entry Agitators' }]} />
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {sideEntrySeries.map((series) => (
              <article
                ...
                <Link to={`/products/side-entry-agitators/${series.slug}`} className="relative block overflow-hidden bg-mist">
                  <SmartImage
                    src={series.image}
                    alt={series.name}
                    ratio="aspect-[4/3]"
